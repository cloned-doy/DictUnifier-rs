...
use super::html::{clean_xml, html2text};
...
fn cover_dict_data_to_xml(text: String, t: char) -> Result<String> {
    match t {
        'h' => {
            ...
            let xml_vec: Vec<_> = html_vec
                .iter()
                .map(|s| s.trim())
                .filter(|s| s.len() > 0)
                .map(|s| format!("<p class=\"plaintext\">{}</p>", s))
                .collect();
                ...
                match t {
                    'W' | 'P' | 'X' => {
                        ...
                        res.push(String::from(
                            "<p class=\"error\"> DictUnifier: Media file is not supported. </p>",
                        ));
                    }
                    'r' => {
                        // Resource file
                        ...
                        res.push(String::from(
                            "<p class=\"error\"> DictUnifier: Resource file is not supported. </p>",
                        ));
                    }
                    _ => {
                        while pos + l < buffer.len() && buffer[pos + l] != 0 {
...
impl Dictionary {
    ...
    pub fn load_idx(&mut self) -> Result<()> {
        ...
        for id in 0..self.info.wordcount {
            ...
            self.idx
                .push(Idx::new(id, index, offset as u32, size as u32));
